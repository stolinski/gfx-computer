...
src/lib/
  platform/                      # the engine shell (state, schema, render host, UI)
    ...
    KineticWordInspector.svelte  # static word, placement, appearance, and phrase authoring
...
interface SurfaceState {
	...
	typeField?: KineticTypeField; // bounded Kinetic Word pool + semantic phrases (ADR-0063)
	enter?: Transition;
	exit?: Transition;
...
- **Additional Block vocabulary** — `paragraph`, the five Diagram Blocks, the five Chart Blocks, and the static `kinetic-word` Block/Type Field substrate ship ([ADR-0063](adr/0063-kinetic-type-word-fields.md)). Independent Kinetic Word tracks and Motion Beats remain designed but not built. `code` and `image` are not registered; mermaid-style auto-layout remains explicitly rejected.
