...
- **Additional Block vocabulary** — `paragraph`, the five diagram Blocks, and the five chart Blocks ship. The Type Field and `kinetic-word` Block are designed but not built ([ADR-0063](adr/0063-kinetic-type-word-fields.md)); only their reusable variable-weight TextAnimation substrate ships so far. `code` and `image` are not registered; mermaid-style auto-layout remains explicitly rejected.
